...
class JochreAnalyzerForSearch(
  ...
  override def finalFilter(tokens: TokenStream): TokenStream = {
    (textNormalizingFilter(_))
      ...
      .andThenIf(log.isTraceEnabled)(tapFilter(log, "postTokenization"))
      .andThen(postTokenizationFilter)
      .andThenIf(log.isTraceEnabled)(tapFilter(log, "postTokenizationFilter"))
      .andThenIf(!forPhrases)(stopWordFilter)
      .andThenIf(log.isTraceEnabled)(tapFilter(log, "stopWordFilter"))
      .andThenIf(addSynonyms)(synonymFilter)
      .andThenIf(log.isTraceEnabled)(tapFilter(log, "synonymFilter"))
      .andThenIf(forPhrases)(skipWildcardFilter)
      .andThenIf(log.isTraceEnabled)(tapFilter(log, "skipWildcardFilter"))
